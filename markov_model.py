...
def top7_markov(df):
    ...
    top7_pos1 = [k for k, _ in freq_ribuan.most_common(7)]
    while len(top7_pos1) < 7:
    ...
    for i in range(3):
        ...
        top7 = [int(k) for k, _ in kandidat[:7]]
        while len(top7) < 7:
...
def top7_markov_order2(df):
    ...
    top_pairs = Counter(pairs).most_common(7)
    d1, d2 = top_pairs[0][0][0], top_pairs[0][0][1]

    top7_d1 = list(set([int(p[0][0]) for p in top_pairs]))
    top7_d2 = list(set([int(p[0][1]) for p in top_pairs]))
    while len(top7_d1) < 7:
    ...
    top7_d3 = [int(k) for k, _ in top7_d3[:7]]
    while len(top7_d3) < 7:
    ...
    top7_d4 = [int(k) for k, _ in top7_d4[:7]]
    while len(top7_d4) < 7:
...
def top7_markov_hybrid(df):
    ...
    for i in range(4):
        ...
        freq = Counter(gabung)
        top7 = [k for k, _ in freq.most_common(7)]
        while len(top7) < 7:
            top7.append(random.randint(0, 9))
        hasil.append(top7)
