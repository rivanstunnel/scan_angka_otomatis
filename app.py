...
# --- UI START ---
st.image("http://googleusercontent.com/file_content/1", width=200)
...
if st.session_state.get('prediction_data') is not None:
    ...
    with st.expander("⬇️ Tampilkan & Unduh Hasil Kombinasi"):
        ...
        # ==== PERBAIKAN: Urutan tab diubah menjadi 2D, 3D, 4D ====
        tab2d, tab3d, tab4d = st.tabs([
            f"Kombinasi 2D ({len(kombinasi_2d_list)})", 
            f"Kombinasi 3D ({len(kombinasi_3d_list)})", 
            f"Kombinasi 4D ({len(kombinasi_4d_list)})"
        ])
        
        # Blok konten juga diubah urutannya
        with tab2d:
